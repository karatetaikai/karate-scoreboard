...
function startTimer(){ paused=false; if(timerInterval) clearInterval(timerInterval); timerInterval=setInterval(()=>{
...
function toggleFirst(btnId){
    ...
    state[key] = !state[key];
    btn.style.backgroundColor = state[key] ? (side==='red'?'red':'blue') : '#888';
}
