...
function updateScores(){ ['ippon','waza','yuko','penalty'].forEach(t=>{document.getElementById('red-'+t).textContent=state.red[t]; document.getElementById('blue-'+t).textContent=state.blue[t];}); const r=state.red.ippon*3+state.red.waza*2+state.red.yuko; const b=state.blue.ippon*3+state.blue.waza*2+state.blue.yuko; document.getElementById('red-score').textContent=r; document.getElementById('blue-score').textContent=b; }
function changeScore(side,type,delta){ let v=state[side][type]+delta; if(type==='penalty'){ v=Math.max(0,Math.min(5,v)); } else { v=Math.max(0,v);} state[side][type]=v; updateScores(); }

// First-take
function toggleFirst(id){ const btn=document.getElementById(id); const red=id==='first-red'; state[red?'firstRed':'firstBlue']=!state[red?'firstRed':'firstBlue']; btn.classList.toggle('active'); btn.classList.toggle(red?'red':'blue'); }
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelectorAll('.plus-btn').forEach(btn=>btn.addEventListener('click',e=>{const s=e.target.closest('.section'); changeScore(s.dataset.side,s.dataset.type,1);})); 
  document.querySelectorAll('.minus-btn').forEach(btn=>btn.addEventListener('click',e=>{const s=e.target.closest('.section'); changeScore(s.dataset.side,s.dataset.type,-1);})); 
  document.getElementById('score-reset').addEventListener('click',()=>{
    ...
    document.getElementById('first-red').className='first-btn';
    document.getElementById('first-blue').className='first-btn';
    updateScores();
  });
